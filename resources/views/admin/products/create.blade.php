...
@section('subtitle', 'Ajoutez un nouveau produit à votre catalogue')

@section('content')
<div x-data="{
    selectedCategory: '{{ old('category', 'alimentation') }}',
    showPreview: false,
    previewImage: null
}">
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:border-green-600 transition-all cursor-pointer relative"
                            ...
                            <div x-show="!showPreview" class="space-y-2">
                                ...
                                <p class="text-[10px] text-slate-500 font-medium">Cliquez pour télécharger une image</p>
                            ...
                            <div x-show="showPreview" class="space-y-2">
                                ...
                                <p class="text-[10px] text-green-600 font-medium">Image sélectionnée</p>
                            </div>
                        </div>
                        @error('image')
                            <p class="text-red-500 text-[10px] mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-green-600 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-[10px] mt-1">{{ $message }}</p>
                @enderror
            </div>
